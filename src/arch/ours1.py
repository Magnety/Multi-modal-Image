...
class MutualAttention_register(nn.Module):
    ...
    def forward(self, warm_flag, feat1, feat2, cam1, cam2, f_g, f_e):
        ...
        _feat2_color_down = F.interpolate(_feat2_color, size=(self.size, self.size), mode='bilinear',
                                          align_corners=False)
...
class OursNet(nn.Module):
    ...
    def forward(self, warm_flag, x1, x2, cam1, cam2):
        ...
        # print(f2_2.shape)
        g_att_output_2, e_att_output_2, affin_grid2_1, affin_grid2_2, edge_2, color_2, f1_attn, f2_attn, g_att_output_2_before, e_att_output_2_before = self.muatt_1(
            warm_flag, f2_1, f2_2, cam1, cam2, f_g, f_e)
