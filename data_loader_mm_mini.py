...
class ImageFolder(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        """mean = torch.mean(out_img[0:1])
        out_img[0:1] = out_img[0:1]-mean
        abs = torch.abs(out_img[0:1])
        out_img[0:1] = out_img[0:1]/torch.max(abs)

        mean = torch.mean(out_img[1:2])
        out_img[1:2] = out_img[1:2] - mean
        abs = torch.abs(out_img[1:2])
        out_img[1:2] = out_img[1:2] / torch.max(abs)

        mean = torch.mean(out_img[2:3])
        out_img[2:3] = out_img[2:3] - mean
        abs = torch.abs(out_img[2:3])
        out_img[2:3] = out_img[2:3] / torch.max(abs)"""
