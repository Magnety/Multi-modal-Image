...
from arch.resnet import resnet18, resnet34
from arch.fusion_resnet import resnet18 as fusion_resnet
from arch.resnet_camfusion_muatt import resnet18 as resnet_camfusion_muatt
from arch.squeezenet_camfusion_muatt import squeezenet as squeezenet_camfusion_muatt
from arch.squeezenet_camfusion_muatt_1 import squeezenet as ours

from arch.vgg_camfusion_muatt import vgg11_bn as vgg_camfusion_muatt
...
from arch.nasnet import nasnet
from arch.preactresnet import preactresnet18
from arch.resnext import resnext50
from arch.senet import seresnet18
from arch.shufflenetv2 import shufflenetv2
from arch.squeezenet import squeezenet
from arch.vgg import vgg11_bn
...
from arch.mobilenetv2_camfusion_muatt import MobileNetV2 as mobilenetv2_camfusion_muatt
class Solver(object):
    def __init__(self, config, train_loader, valid_loader, test_loader):
        ...
        self.alpha[0,0]=0.5
        self.alpha[1,0]=0.5
        self.action1 = [-0.1,-0.05,0,0.05,0.1]#[-0.005,-0.002,0,0.002,0.005]
    ...
    def build_model(self):
        ...
        if self.model_type == 'resnet18':
            ...
            self.unet = densenet121()
        elif self.model_type == 'googlenet':
            self.unet = googlenet()
        elif self.model_type == 'inceptionv4':
            self.unet = inceptionv4()
        elif self.model_type == 'mobilenetv2':
            self.unet = mobilenetv2()
        elif self.model_type == 'nasnet':
            self.unet = nasnet()
        elif self.model_type == 'preactresnet18':
            self.unet = preactresnet18()
        elif self.model_type == 'resnext50':
            self.unet = resnext50()
        elif self.model_type == 'seresnet18':
            self.unet = seresnet18()
        elif self.model_type == 'shufflenetv2':
            self.unet = shufflenetv2()
        elif self.model_type == 'squeezenet':
            self.unet = squeezenet()
        elif self.model_type == 'vgg11_bn':
            self.unet = vgg11_bn()
        elif self.model_type == 'xception':
            self.unet = xception()

        elif self.model_type == 'fusion_resnet':
            self.unet = fusion_resnet()
        elif self.model_type == 'resnet_camfusion_muatt':
            self.unet = resnet_camfusion_muatt()
        elif self.model_type == 'ours':
            self.unet = ours()
        elif self.model_type == 'mobilenetv2_camfusion_muatt':
            self.unet = mobilenetv2_camfusion_muatt ()
        if self.opt == "Adam":
            ...
            for epoch in range(self.num_epochs):
                ...
                for _images,_label,_name in tqdm(self.train_loader):
                    ...
                    if self.modal == 0:
                        ...
                        if epoch < self.warmup:
                            cls,g_att_output,e_att_output,x1_1,x2_1,affin_grid_1,affin_grid_2,edge,color,g_att,e_att,x1_1_before,x2_1_before = self.unet(torch.zeros((1)), images[:, 0:3, :, :], images[:, 3:6, :, :],
                                                                                                                                 activation_map_g,
                                                                                                                                 activation_map_e)
                        else:
                            cls,g_att_output,e_att_output,x1_1,x2_1,affin_grid_1,affin_grid_2,edge,color,g_att,e_att,x1_1_before,x2_1_before = self.unet(torch.zeros((1)), images[:, 0:3, :, :], images[:, 3:6, :, :],
                    ...
                    if self.modal == 4:
                        if epoch < self.warmup:
                            ...
                            cls,g_att_output,e_att_output,x1_1,x2_1,affin_grid_1,affin_grid_2,edge,color,g_att,e_att,x1_1_before,x2_1_before = self.unet(torch.ones((1)), images[:, 0:3, :, :], images[:, 3:6, :, :],
                                                                                                                                 activation_map[0],
                                                                                                                                 activation_map[1])
                    cls_cpu = cls.data.cpu().numpy()
                    label_cpu = label.data.cpu().numpy()
                    ...
                    state = np.append(cls_cpu[:,1],[np.array(np.sum(label_cpu==0)),np.array(np.sum(label_cpu==1))])
                    ...
                    if epoch > self.warmup:
                        ...
                        print('action1:',self.action1[_action1],'action1:',-self.action1[_action1],'action2:',self.action1[_action2])
                    ...
                    next_state = np.append(cls_cpu[:,1],[np.array(np.sum(label_cpu==0)),np.array(np.sum(label_cpu==1))])
                ...
                if epoch % self.val_step == 0:
                    ...
                    for i, (images, label, name) in enumerate(self.valid_loader):
                        ...
                        if self.modal == 4:
                            if epoch < self.warmup:
                                cls,g_att_output,e_att_output,x1_1,x2_1,affin_grid_1,affin_grid_2,edge,color,g_att,e_att,x1_1_before,x2_1_before = self.unet(torch.zeros((1)),images[:, 0:3, :, :], images[:, 3:6, :, :], activation_map[0],
                                                                                                                                     activation_map[1])
                            else:
                                cls,g_att_output,e_att_output,x1_1,x2_1,affin_grid_1,affin_grid_2,edge,color,g_att,e_att,x1_1_before,x2_1_before = self.unet(torch.ones((1)), images[:, 0:3, :, :], images[:, 3:6, :, :], activation_map[0],
                                                                                                                                     activation_map[1])
                        ...
                        for j in range(1, activation_map[0].shape[0]):
                            am0 = torch.cat([am0, activation_map[0][j]], dim=1)
                        plt.imsave(os.path.join(self.result_dir, '{}_{}_cam_g.png'.format(epoch + 1,i)),
                                   am0.squeeze(0).cpu().numpy())


                        am1 = activation_map[1][0]
                        for j in range(1, activation_map[1].shape[0]):
                            am1 = torch.cat([am1, activation_map[1][j]], dim=1)
                        plt.imsave(os.path.join(self.result_dir, '{}_{}_cam_e.png'.format(epoch + 1,i)),
                                   am1.squeeze(0).cpu().numpy())"""
                        ...
                        grid_map_1(affin_grid_1[0,:,:,:].data.cpu(),os.path.join(self.result_dir, '{}_{}_g_affin.png'.format(epoch + 1, i)))
                        grid_map_1(affin_grid_2[0,:,:,:].data.cpu(),os.path.join(self.result_dir, '{}_{}_e_affin.png'.format(epoch + 1, i)))
                        ...
                        for j in range(0,x1_1.shape[1]):
                            att_f_g_ =  overlay_mask(to_pil_image(denorm(images[0, 0:3,  0:128, 0:128].data)),
                                                     to_pil_image(denorm(x1_1[0,j:j+1,:,:].data), mode='F'), alpha=0.01)
                            ...
                            att_f_g_before_ =  overlay_mask(to_pil_image(denorm(images[0, 0:3,  0:128, 0:128].data)),
                                                     to_pil_image(denorm(x1_1_before[0,j:j+1,:,:].data), mode='F'), alpha=0.01)
                            ...
                            att_f_e_ =  overlay_mask(to_pil_image(denorm(images[0, 3:6,  0:128, 0:128].data)),
                                                     to_pil_image(denorm(x2_1[0,j:j+1,:,:].data), mode='F'), alpha=0.01)
                            ...
                            att_f_e_before_ =  overlay_mask(to_pil_image(denorm(images[0, 3:6,  0:128, 0:128].data)),
                                                     to_pil_image(denorm(x2_1_before[0,j:j+1,:,:].data), mode='F'), alpha=0.01)
